...
**編隊抓取（Formation grab）**：
抓取工具的「編隊」模式：讓單一指標同時驅動一組（可為任意 N 個）Grab，彼此相對位置是使用者預先在畫布上互動式擺定的固定偏移量，拖曳時整組偏移量不隨方向旋轉、只整體跟著指標平移；放開時每一點各自帶自己的 Fling 速度離手。是產生 Multi-grab 的第三種手段（見上）——差別在於單一指標即時操作、且點與點間距是使用者精準設定，而非天然多指的手感。
_Avoid_: 雙抓取工具（只精確描述 N=2 的特例，非通用名稱）、rigid grab、grab pattern

**大把抓取（Handful grab）**：
抓取工具的「大把」模式：按下時以指標為圓心、可調半徑內、落在被點中那塊 Jelly 上的 Particle 一次抓成一把，拖曳時整把跟著指標平移、不旋轉；中心跟得最緊、越往邊緣越鬆，讓被抓的一團保持形狀、邊緣柔順地融進本體——拉得很遠時，被拉長的部分從這一團的邊緣開始，而不是從一個點抽成細絲（見 ADR-0014）。抓到哪些 Particle 在按下當下就定了，拖曳途中改半徑不影響已抓住的這一把；半徑圈不跨到別塊 Jelly。快速按放＝以同一半徑對該處 Tap。這個模式下不能釘選。一把算一次抓取（放開就是整把一起放開、帶 Fling 離手），**不屬於** Multi-grab。
_Avoid_: 範圍抓取（「範圍」已被電風扇與撒 Pin 用掉）、area grab、多點抓取、編隊抓取（那是固定偏移的 N 個彼此獨立的 Grab，不是一團）
