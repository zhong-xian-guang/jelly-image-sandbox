...
**大把抓取（Handful grab）**：
一個沙盒工具：按下時以指標為圓心、可調半徑內、落在被點中那塊 Jelly 上的 Particle 一次抓成一把，拖曳時整把跟著指標平移、不旋轉；中心跟得最緊、越往邊緣越鬆，讓被抓的一團保持形狀、邊緣柔順地融進本體——拉得很遠時，被拉長的部分從這一團的邊緣開始，而不是從一個點抽成細絲（見 ADR-0014）。抓到哪些 Particle 在按下當下就定了，拖曳途中改半徑不影響已抓住的這一把；半徑圈不跨到別塊 Jelly。快速按放＝以同一半徑對該處 Tap。這個工具下不能釘選。一把算一次抓取（放開就是整把一起放開、帶 Fling 離手），**不屬於** Multi-grab。
...
**提示（Hint）**：
疊在畫布上、只為了讓使用者看懂場上狀態的視覺層，本身不參與物理也不接手勢：顯示網格（debug 線框）、Pin 標記、電風扇的矩形範圍與圖示、編隊抓取的形狀標記、大把抓取的範圍圈。每一種各有獨立的顯示開關，另有一個全域的「播放時隱藏提示」——Track／Demo 播放中一律暫時壓下、播完各自回到原本的開關狀態，避免多條疊播時畫面雜亂。撒 Pin／移除 Pin 的筆刷圓圈**不算**提示：它是「目前工具的作用範圍在哪」的游標，跟著工具選擇走、不受該全域開關影響。大把抓取的範圍圈外觀相同、也跟著游標走，卻歸在提示：它在拖曳全程都有意義，錄素材時會壓在果凍上入鏡，所以需要自己的開關、也要在播放時被壓下。
