...
**Multi-grab（多重抓取）**：
同時存在的一組彼此獨立的 Grab。三種產生方式：v1 由多指／多指標即時操作產生；v2 素材工具可疊加播放多條各自單指標錄製的 Action Track 做出同樣效果；抓取工具的編隊模式（編隊抓取）讓單一指標一次驅動一組固定相對位置的 Grab，做出精準、可重複的同時多點操作。
_Avoid_: 多點觸控抓取

**Fan（電風扇）**：
一個沙盒工具，是模擬核心繼 Grab/Pin（點約束）、Tap（一次性徑向脈衝）之後第三種力學機制：對一個矩形涵蓋範圍逐 substep 持續施力，而非瞬時或約束式。矩形一端是風扇面，推力沿吹風方向從面往外、依距離冪次衰減；強度、矩形長寬、衰減程度皆為可調參數。畫布上按下拖曳＝放置：按下點為風扇面原點、拖曳方向＝吹風方向、拖曳距離＝矩形長度；右鍵單擊風扇＝移除。v1 場上同時只有一個 Fan（刻意從簡，非 Pin 那種數量不限）。錄製進 Action Track。
_Avoid_: wind、force field（目前只有這一種持續力場工具，不需要更抽象的詞）、風力

**工具（Tool）**：
決定畫布上滑鼠手勢怎麼解讀的一組操作，同時只有一個作用中：抓取、Pin、電風扇、Jelly。從工具列切換。
_Avoid_: 目前工具下拉、沙盒工具（舊稱）、一般操作（那是抓取工具的單點模式）

**模式（Tool mode）**：
同一個工具底下，左鍵行為的幾種變體（例如抓取工具的單點／大把／編隊、Pin 工具的放／拔）。電風扇是例外：它的模式（寬度／強度／衰減／頻率）不改左鍵，只決定「按住右鍵＋滾輪」調哪個參數。在畫布上中鍵單擊依序輪替；每個工具各自記得自己目前的模式。
_Avoid_: 子工具、子模式、狀態

**編隊抓取（Formation grab）**：
抓取工具的「編隊」模式：讓單一指標同時驅動一組（可為任意 N 個）Grab，彼此相對位置是使用者預先在畫布上互動式擺定的固定偏移量，拖曳時整組偏移量不隨方向旋轉、只整體跟著指標平移；放開時每一點各自帶自己的 Fling 速度離手。是產生 Multi-grab 的第三種手段（見上）——差別在於單一指標即時操作、且點與點間距是使用者精準設定，而非天然多指的手感。可開啟**每點大把**：每個點改成一把大把抓取（半徑共用大把抓取的半徑），整組變成 N 把固定相對位置、彼此獨立的一團；快速按放時每個點各以同一半徑 Tap。
_Avoid_: 雙抓取工具（只精確描述 N=2 的特例，非通用名稱）、rigid grab、grab pattern

**大把抓取（Handful grab）**：
抓取工具的「大把」模式：按下時以指標為圓心、可調半徑內、落在被點中那塊 Jelly 上的 Particle 一次抓成一把，拖曳時整把跟著指標平移、不旋轉；中心跟得最緊、越往邊緣越鬆，讓被抓的一團保持形狀、邊緣柔順地融進本體——拉得很遠時，被拉長的部分從這一團的邊緣開始，而不是從一個點抽成細絲（見 ADR-0014）。抓到哪些 Particle 在按下當下就定了，拖曳途中改半徑不影響已抓住的這一把；半徑圈不跨到別塊 Jelly。快速按放＝以同一半徑對該處 Tap。一把算一次抓取（放開就是整把一起放開、帶 Fling 離手），**不屬於** Multi-grab。
_Avoid_: 範圍抓取（「範圍」已被電風扇與撒 Pin 用掉）、area grab、多點抓取、編隊抓取（那是固定偏移的 N 個彼此獨立的 Grab；開啟每點大把時是 N 把彼此獨立的一團，仍不是單一一把）

**撒 Pin（Pin spray）**：
Pin 工具「放」模式下的左鍵拖曳：沿拖曳路徑、以 Pin 筆刷半徑為範圍，依密度（新 Pin 與既有 Pin 之間的最小間距）持續撒下 Pin，只落在能附著到 Jelly 表面（某三角形內）的位置。同模式下左鍵單擊只放一顆。
_Avoid_: 噴灑、批次 Pin

**移除 Pin（Pin eraser）**：
Pin 工具「拔」模式下的左鍵拖曳：經過路徑上（以 Pin 筆刷半徑為範圍）的既有 Pin 被持續移除，行為類似橡皮擦；不影響未鎖定的 Grab。
_Avoid_: 清除、批次移除（與既有「清除 Pin 事件」不同——那是一次清空全部，這是依筆刷範圍局部移除）

**Pin 筆刷半徑（Pin brush radius）**：
撒 Pin 與移除 Pin 共用的一個作用半徑；按住右鍵＋滾輪調整。
_Avoid_: 撒 Pin 半徑、橡皮擦半徑（兩者已合一）

**提示（Hint）**：
疊在畫布上、只為了讓使用者看懂場上狀態的視覺層，本身不參與物理也不接手勢：顯示網格（debug 線框）、Pin 標記、電風扇的矩形範圍與圖示、編隊抓取的形狀標記、大把抓取的範圍圈。每一種各有獨立的顯示開關，另有一個全域的「播放時隱藏提示」——Track／Demo 播放中一律暫時壓下、播完各自回到原本的開關狀態，避免多條疊播時畫面雜亂。Pin 筆刷圓圈與游標標籤（游標旁顯示目前模式與滾輪調到的數值）**不算**提示：它們是「目前工具在做什麼」的游標回饋，跟著工具選擇走、不受該全域開關影響（游標標籤另有自己的開關）。大把抓取的範圍圈外觀相同、也跟著游標走，卻歸在提示：它在拖曳全程都有意義，錄素材時會壓在果凍上入鏡，所以需要自己的開關、也要在播放時被壓下。
_Avoid_: overlay（指的是實作手法——DOM 覆蓋層，不是這個概念）、HUD、輔助線

**乾淨畫面（Clean view）**：
為了用外部螢幕錄影錄素材而切換的顯示狀態：畫布上只剩 Jelly——側欄、播放控制條、相機按鈕、匯入提示字、所有提示與游標回饋（筆刷圓圈、游標標籤）一律藏起來，離開後各自回到原本的狀態。跟「播放時隱藏提示」不同：那只壓提示、只在播放中生效；乾淨畫面是使用者手動進出、什麼都藏。
_Avoid_: 全螢幕（那是瀏覽器視窗的事）、錄影模式、隱藏 UI
...
**Pin（釘選）**：
把 Jelly 表面某一點絕對鎖定在某個世界座標的約束——就是「目標點被凍結、不再跟指標」的 Grab。數量不限；用力甩、Tap 都拔不掉。定位是擺設場景：放下就固定，不拖曳搬移（要換位置就拔掉重放）。沒有「鎖定質心」的獨立模式，要固定中心就在附近放幾個 Pin（見 ADR-0004）。
...
**重建（Rebuild）**：
用目前的匯入尺寸與網格密度拉霸值，從來源圖重新生成場上 Jelly 的網格；擺放位置不變，但那塊會回到 rest 形狀、附在它上面的 Pin／Grab 掉光。兩種入口：「全部重建」按鈕一次重建場上每一塊；Jelly 工具右鍵選單的「重建」只重建點中的那一塊。重建只改 Scene，不是 Track 事件，錄製中不可用。
...
**生成 Jelly（Spawn）**：
Jelly 工具的左鍵單擊：以該點為中心放下一塊新的 Jelly，圖 = 最近一次匯入的圖（還沒匯入過就是內建預設果凍），尺寸與密度 = 當下兩條拉霸的值。Walled／Floor 邊界外的位置拒絕生成。不在錄製中＝改 Scene；錄製中＝錄進 Action Track 當事件，播到那一步才出現、重設後不存在。
_Avoid_: 放置（Fan 的手勢用語）、複製、clone、stamp

**移除 Jelly（Remove）**：
Jelly 工具下，右鍵單擊某塊 Jelly 跳出的選單項：移除那一塊，連同附在它上面的 Pin／Grab。左鍵點在既有 Jelly 上仍是生成，移除只走右鍵選單，所以「貼著生成」不會誤刪。不在錄製中＝改 Scene；錄製中＝錄進 Action Track 當事件。
