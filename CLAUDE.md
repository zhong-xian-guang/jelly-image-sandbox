...
- **釘選（Pin）** — 把果凍表面上任意一點絕對鎖定在原地，數量不限（見 `docs/adr/0004`）。用力甩、輕拍都拔不掉；定位是擺設場景，放下就固定、不拖曳搬移（要換位置就拔掉重放）。在一角或幾處放 Pin、再拉另一角，就能觀察物體被拉到極限的穩定狀態。（沒有「鎖定質心」的獨立模式——要固定中心就在附近多放幾個 Pin。）
